  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
      ...
      <div className="relative h-64 sm:h-80">
        {/* Y-axis labels */}
        <div className="absolute left-0 top-0 h-full flex flex-col justify-between text-xs text-gray-500 w-16 sm:w-20">
        ...
        {/* Chart area */}
        <div className="ml-16 sm:ml-20 h-full flex items-end justify-between space-x-1 sm:space-x-3">
          {data.map((item, index) => {
            const height = (item.amount / maxAmount) * 100;
            return (
              <div key={index} className="flex flex-col items-center flex-1 h-full justify-end group min-w-0">
                <div className="relative w-full">
                  {/* Tooltip */}
                  <div className="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-2 sm:px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 shadow-lg">
                ...
                {/* Week label */}
                <span className="text-xs sm:text-sm text-gray-600 mt-2 sm:mt-4 font-medium truncate w-full text-center">
                  {item.week}
                </span>
              </div>
            );
